...
with open(f'statis/ISE_res.csv', 'w', newline='') as csvfile:
    # 建立 CSV 檔寫入器
    writer = csv.writer(csvfile)

    writer.writerow(['ISE', '', 'Cipher NPCR', '', '', 'Cipher UACI', '', ''])
    ...
    for i in range(len(origin_image_files)):
        ...
        # 構建完整的文件路徑
        encrypt_image_path_2 = os.path.join(encrypt_folder_path, origin_image_files[i][:-4] + '_enc_ISE' + origin_image_files[i][-4:]) 
        ...
        if len(encrypt_image_1.shape) == 3:
            ...
            # 寫入csv
            writer.writerow([origin_image_files[i], 'color', str(cipher_NPCR_red), str(cipher_NPCR_green), str(cipher_NPCR_blue), str(cipher_UACI_red), str(cipher_UACI_green), str(cipher_UACI_blue)])
        else:
            # 計算NPCR
            ...
            # 寫入csv
            writer.writerow([origin_image_files[i], 'gray', str(cipher_NPCR), '', '', str(cipher_UACI), '', ''])
